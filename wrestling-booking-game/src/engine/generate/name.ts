...
import { FIRST_NAMES, SURNAMES, EPITHET_NOUNS, EPITHET_ADJECTIVES, NAME_TITLES } from '../../data/names';
...
function generateCandidate(rng: Rng): string {
  ...
  switch (pattern) {
    ...
    case 'titled':
      return `${pick(rng, NAME_TITLES)} ${pick(rng, FIRST_NAMES)}`;
  }
}
 ...
 */
export function generateName(rng: Rng, existingNames: ReadonlySet<string>): string {
