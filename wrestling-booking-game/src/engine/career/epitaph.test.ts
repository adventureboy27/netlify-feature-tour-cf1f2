...
import { howTheyWent, whatTheyLeave, whoTheyWere } from './epitaph';
...
describe('who he was, in one clause', () => {
  it('leads with the hall when he is in it', () => {
    expect(whoTheyWere(man({ careerStatus: 'hallOfFamer' }), 2020)).toBe('Hall of Fame');
  });

  it('counts the reigns when there were several', () => {
    const reigns = [1, 2, 3, 4].map((i) => ({ titleId: 't1', startWeek: i * 10, endWeek: i * 10 + 5 }));
    expect(whoTheyWere(man({ titleReigns: reigns as never }), 2020)).toBe('4-time champion');
  });
  ...
  it('has something to say about a man who never won anything', () => {
    ...
    const said = whoTheyWere(man({ career: { ...man().career, matches: 300 } }), 2020);
...
describe('what he leaves', () => {
  ...
  it('says how long he carried one when it was a real run', () => {
    const left = whatTheyLeave(man({ titleReigns: [{ titleId: 't1', startWeek: 100, endWeek: 300 }] as never }), ctx);
    expect(left.join(' ')).toMatch(/200 weeks carrying/);
  });

  it('still has something for a man with no belts and no highlights', () => {
    const left = whatTheyLeave(man({ record: { wins: 120, losses: 300, draws: 4 } }), ctx);
  ...
  it('is a short list, never a wall of statistics', () => {
    ...
    expect(whatTheyLeave(everything, ctx).length).toBeLessThanOrEqual(4);
  });
});
...
describe('how he went', () => {
  it('says it plainly when the company was what killed him', () => {
    ...
    expect(howTheyWent(killed, 'An accident.')).toContain('did not come back');
  });
  ...
  it('leaves an ordinary passing alone', () => {
    ...
    expect(howTheyWent(passed, 'suddenly, of a heart attack')).toBe('Died suddenly, of a heart attack.');
  });
});
